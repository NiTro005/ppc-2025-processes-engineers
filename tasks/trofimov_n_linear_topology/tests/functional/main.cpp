...
class TrofimovNLinearTopologyFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    {
      std::string abs_path = ppc::util::GetAbsoluteTaskPath(PPC_ID_trofimov_n_linear_topology, "pic.jpg");
...
TEST_P(TrofimovNLinearTopologyFuncTests, MatmulFromPic) {
...
const auto kTestTasksList =
    std::tuple_cat(ppc::util::AddFuncTask<TrofimovNLinearTopologyMPI, InType>(kTestParam, PPC_SETTINGS_trofimov_n_linear_topology),
...
INSTANTIATE_TEST_SUITE_P(PicMatrixTests, TrofimovNLinearTopologyFuncTests, kGtestValues, kPerfTestName);
